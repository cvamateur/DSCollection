...
class DataExtractor:
        ...
        with ThreadPoolExecutor(max_workers=self.num_workers) as executor:
            for info in cvt_results:
                ...
                if contiguous:
                    ...
                    dstImg = os.path.join(dstImgDir, nameFmt.format(info.index, ext))
    ...
    def create_dataset(root: str, name: str, imgDir: str, lblDir: str):
        ...
        root = check_path(os.path.join(dstDir, name), False)
        os.makedirs(os.path.join(root, imgDir))
        os.makedirs(os.path.join(root, lblDir))
        return root
